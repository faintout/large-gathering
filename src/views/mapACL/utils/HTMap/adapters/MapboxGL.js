...
export default class MapboxGL extends BaseAdapter {
  constructor(containerId, options) {
    ...
    // 默认关闭坐标转换，Mapbox 环境通常使用 WGS84，强制转换会导致偏移
    this._enableCoordinateTransform = this.options.enableCoordinateTransform === true
    this.init()
  }
  ...
  /**
   * 添加轨迹 - 实时显示车辆轨迹
   * @param {object} options - { id, path, symbol?, lineStyle? }
   * @returns {object|null}
   */
  addTrack(options) {
   ...
   */
  updateTrack(id, path) {
    return this.trackManager.updateTrack(id, path)
  }
   ...
   */
  pushTrackPoint(id, lngLat) {
    return this.trackManager.pushTrackPoint(id, lngLat)
  }
  ...
  addDomMarker(options) {
    ...
    try {
      ...
      const marker = new mapboxgl.Marker({
        ...
        rotationAlignment: options.rotationAlignment || 'auto',
        pitchAlignment: options.pitchAlignment || 'auto'
      })
  ...
  addLines(options) {
    ...
    try {
      ...
      // ========== 检测重复 ID ==========
      // 1. 检测 options.geometries 内部是否有重复的 id
      const geometryIds = validatedGeometries.map(g => g.id)
      const duplicateIds = geometryIds.filter((id, index) => geometryIds.indexOf(id) !== index)
      if (duplicateIds.length > 0) {
        const uniqueDuplicates = [...new Set(duplicateIds)]
        console.warn(`⚠️ 警告: options.geometries 中存在重复的 id:`, uniqueDuplicates)
        console.warn(`重复 id 详情:`, uniqueDuplicates.map(id => ({
          id,
          出现次数: geometryIds.filter(gid => gid === id).length
        })))
      }

      // 2. 检测新添加的 id 是否在已存在的线段组中
      const existingLineIds = []
      this.layers.forEach((layer, layerId) => {
        // 检查是否是线段组（有 geometries 属性且是数组）
        if (layer.geometries && Array.isArray(layer.geometries)) {
          layer.geometries.forEach(geo => {
            if (geometryIds.includes(geo.id)) {
              existingLineIds.push({
                新线段id: geo.id,
                已存在于线段组: layer.id || layerId
              })
            }
          })
        }
      ...
      if (existingLineIds.length > 0) {
        console.warn(`⚠️ 警告: 新添加的线段 id 在已存在的线段组中发现重复:`)
        console.table(existingLineIds)
      }
      // ========== 检测结束 ==========
