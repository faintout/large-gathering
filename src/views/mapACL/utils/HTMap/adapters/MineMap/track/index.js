 ...
 * 接口：updateTracks、stopTracks、removeTracks、setInfoWindowVisible、setStyles、setZoomRange
 */
...
/** 将 contentDom（字符串或 DOM）渲染到 label 容器内，样式为左右对齐（标签左、数值右） */
function renderLabelContent(label, contentDom) {
  label.textContent = ''
  label.style.textAlign = 'left'
  if (contentDom == null) return
  if (typeof contentDom === 'string') {
    const lines = contentDom.split('\n').filter(Boolean)
    lines.forEach((line) => {
      ...
      if (sep >= 0) {
        const left = document.createTextNode(line.slice(0, sep + 1))
        const right = document.createElement('span')
        right.style.color = '#fff'
        // right.style.fontWeight = '600'
        right.style.flexShrink = '0'
        right.textContent = line.slice(sep + 1)
  ...
  if (contentDom instanceof HTMLElement) {
    const clone = contentDom.cloneNode(true)
  ...
  label.textContent = String(contentDom)
}
...
export default class TrackManager {
    ...
    payload.itemsData.forEach(item => {
      ...
      if (!track) {
        track = {
          ...
          path: [transformCoordinates(lngLat, `track-${id}-init`)],
          markerInstance: null,
          lineSourceId: null,
          lineLayerId: null,
      ...
      track.path.push(transformCoordinates(lngLat, `track-${id}-update`))
  ...
  _createVehicleElement(track) {
    ...
    renderLabelContent(label, track.contentDom)
